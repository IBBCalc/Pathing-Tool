...
export default class PartyUiHandler extends MessageUiHandler {
            ...
            if (this.selectCallback && this.partyUiMode !== PartyUiMode.CHECK) {
              if (option === PartyOption.TRANSFER) {
                if (this.transferCursor !== this.cursor) {
                  if (this.transferAll) {
                    getTransferrableItemsFromPokemon(globalScene.getPlayerParty()[this.transferCursor]).forEach((_, i) => (this.selectCallback as PartyModifierTransferSelectCallback)(this.transferCursor, i, this.transferQuantitiesMax[i], this.cursor, true));
                  } else {
  ...
  clearTransfer(): void {
    ...
    for (let i = 0; i < this.partySlots.length; i++) {
      ...
      if (globalScene.pathingToolUI) this.partySlots[i].slotHpPercentageText.setVisible(true);
      if (globalScene.pathingToolUI) this.partySlots[i].slotEtherText.setVisible(true);
    }
  }
...
class PartySlot extends Phaser.GameObjects.Container {
  ...
  setup(partyUiMode: PartyUiMode, tmMoveId: Moves) {
    ...
    if (partyUiMode !== PartyUiMode.TM_MODIFIER) {
      ...
      if (globalScene.pathingToolUI) this.slotHpPercentageText.setVisible(true);
      if (globalScene.pathingToolUI) this.slotEtherText.setVisible(true);
    } else {
      this.slotHpBar.setVisible(false);
      this.slotHpOverlay.setVisible(false);
      this.slotHpText.setVisible(false);
