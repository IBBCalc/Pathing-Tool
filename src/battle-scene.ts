...
import { ConsumableModifier, ConsumablePokemonModifier, DoubleBattleChanceBoosterModifier, ExpBalanceModifier, ExpShareModifier, ExtraModifierModifier, FusePokemonModifier, GigantamaxAccessModifier, HealingBoosterModifier, HiddenAbilityRateBoosterModifier, LockModifierTiersModifier, MegaEvolutionAccessModifier, ModifierBar, MoneyMultiplierModifier, MultipleParticipantExpBonusModifier, PersistentModifier, PokemonExpBoosterModifier, PokemonFormChangeItemModifier, PokemonHeldItemModifier, PokemonHpRestoreModifier, PokemonIncrementingStatModifier, RememberMoveModifier, TerastallizeAccessModifier, TerastallizeModifier } from "./modifier/modifier";
...
export default class BattleScene extends SceneBase {
  ...
  * - false: Show hints for moves
   ...
   */
  randBattleSeedInt(range: number, min: number = 0, reason?: string): number {
    return this.currentBattle?.randSeedInt(range, min, reason);
  }
    ...
    if (!this.getEnemyField()[0].hasTrainer()) {
      this.getEnemyField().forEach((pk, i) => {
        if (pk.isActive() && pk.hp > 0) {
          txt = txt.concat(LoggerTools.findBest(pk));
        }
      });
    }
    if (txt.length > 2) {
  ...
  getSpeciesFormIndex(species: PokemonSpecies, gender?: Gender, nature?: Nature, ignoreArena?: boolean): number {
    ...
    switch (species.speciesId) {
      ...
      case Species.URSHIFU:
        return Utils.randSeedInt(2, 0, "Species Form Urshifu");
